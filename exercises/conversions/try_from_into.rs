...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if !is_valid_rgb_value(tuple.0) {
            return Err(String::from("invalid RBG value"));
        }
        if !is_valid_rgb_value(tuple.1) {
            return Err(String::from("invalid RBG value"));
        }
        if !is_valid_rgb_value(tuple.2) {
            return Err(String::from("invalid RBG value"));
        }
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if !arr.iter().all(|rgb_value| is_valid_rgb_value(*rgb_value)) {
            return Err(String::from("invalid RBG value"));
        }
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        if !slice.iter().all(|rgb_value| is_valid_rgb_value(*rgb_value)) {
            return Err(String::from("invalid RBG value"));
        }
